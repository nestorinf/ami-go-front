  ...
  <v-card class="mb-7">
    <v-card-text class="pa-5 border-bottom">
      ...
      <h6 class="subtitle-2 font-weight-light">
        En este formulario se registran todas las restricciones sobre una zona
        de cobertura
      </h6>
    </v-card-text>
    <v-card-text>
...
export default {
  ...
  methods: {
    ...mapActions({
      createGeofenceRestriction:
        "geofenceRestriction/createGeofenceRestriction",
      getGeofenceRestrictionById:
        "geofenceRestriction/getGeofenceRestrictionById",
      updateGeofenceRestriction:
        "geofenceRestriction/updateGeofenceRestriction",
      getDepartmentsData: "department/getDepartmentsData",
      getMunicipalityByDepartment: "municipality/getMunicipalityByDepartment",
      getGeofenceByDepartmentMunicipality:
        "geofence/getGeofenceByDepartmentMunicipality",
      getGeofenceRestrictionsByGeofence:
        "geofenceRestriction/getGeofenceRestrictionsByGeofence",
    }),
    ...
    Geofence() {
      this.geofence = [];
      const pathgeofence = this.geofenceList.filter(
        (geofence) => geofence.value === this.form.geofence_id
      );
      this.geofence.push({
    ...
    loadGeofenceByDepartmentMunicipality() {
      ...
      const geofences = [];
      this.geofenceList = [];
      this.getGeofenceByDepartmentMunicipality({
    ...
    loadGeofenceRestrictionsByGeofence() {
      this.getGeofenceRestrictionsByGeofence(this.form.geofence_id)
        .then((result) => {
          ...
          if (result) {
            ...
            const pathregisteredRestrictions = this.restrictionList.filter(
              (restriction) => restriction.value != this.id
            );
