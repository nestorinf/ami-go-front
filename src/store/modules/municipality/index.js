...
const actions = {
  ...
  getMunicipalityByDepartment({ commit, dispatch }, department_id) {
    return new Promise((resolve, reject) => {
      dispatch("loading/loadingState", true, { root: true });
      MunicipalityService.allMunicipalityByDepartment(department_id)
        .then(({ data }) => {
          const municipality = data.payload;

          commit("setMunicipalities", municipality);
          dispatch("loading/loadingState", false, { root: true });
          resolve(data.payload);
        })
        .catch((err) => {
  ...
  createMunicipality({ commit, dispatch }, body) {
    return new Promise((resolve, reject) => {
      dispatch("loading/loadingState", true, { root: true });
      MunicipalityService.create(body)
        .then(({ data }) => {
  ...
  updateMunicipality({ commit, dispatch }, body) {
    return new Promise((resolve, reject) => {
      dispatch("loading/loadingState", true, { root: true });
      MunicipalityService.update(body)
        .then(({ data }) => {
          const municipality = Object.assign({}, data.payload);
